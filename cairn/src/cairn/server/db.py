...
def _ensure_fact_columns(conn: sqlite3.Connection) -> None:
    """Ensure facts table has exploit detail columns for vulnerability logging."""
    columns = {row["name"] for row in conn.execute("PRAGMA table_info(facts)")}
    migrations = [
        ("exploit_type", "ALTER TABLE facts ADD COLUMN exploit_type TEXT"),
        ("target_host", "ALTER TABLE facts ADD COLUMN target_host TEXT"),
        ("target_port", "ALTER TABLE facts ADD COLUMN target_port TEXT"),
        ("success", "ALTER TABLE facts ADD COLUMN success INTEGER NOT NULL DEFAULT 0"),
        ("request_data", "ALTER TABLE facts ADD COLUMN request_data TEXT"),
        ("response_data", "ALTER TABLE facts ADD COLUMN response_data TEXT"),
    ]
