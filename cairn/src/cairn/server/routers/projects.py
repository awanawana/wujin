...
def get_project(project_id: str):
    with get_conn() as conn:
        ...
        def fact_row_to_model(f: sqlite3.Row) -> Fact:
            data = dict(f)
        ...
        return ProjectDetail(
            ...
            facts=[fact_row_to_model(f) for f in facts],
...
def get_project_logs(project_id: str, lines: int = 50):
    """获取指定项目容器的最新日志"""
    container_name = f"cairn-dispatch-proj_{project_id}"
    try:
        ...
        if result.returncode != 0:
            # 容器不存在，返回已完成/不存在提示
            all_proj = subprocess.run(
                ["docker", "ps", "-a", "--filter", f"name={container_name}", "--format", "{{.Names}}"],
                capture_output=True, text=True
            )
            if container_name not in all_proj.stdout:
                return {"logs": f"项目 {project_id} 容器已清理或不存在", "running": False}
            # 容器已停止，看退出日志
            logs = result.stdout + result.stderr
            return {"logs": logs if logs else f"项目 {project_id} 已完成，无运行日志", "running": False}
        logs = result.stdout + result.stderr
        return {"logs": logs, "running": True}
    except Exception as e:
        return {"logs": f"获取日志失败: {str(e)}", "running": False}
