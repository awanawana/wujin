...
def conclude(project_id: str, intent_id: str, body: ConcludeRequest):
    with get_conn() as conn:
        ...
        conn.execute(
            "INSERT INTO facts (id, project_id, description, exploit_type, target_host, target_port, success, request_data, response_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (fid, project_id, body.description, body.exploit_type, body.target_host, body.target_port, 1 if body.success else 0, body.request_data, body.response_data),
        )
        conn.execute(
