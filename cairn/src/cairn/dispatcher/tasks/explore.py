    ...
    try:
        ...
        if not did_timeout(first) and first.returncode == 0:
            ...
            return write_conclude_result(
                ...
                intent.id,
                worker.name,
                description["description"] if isinstance(description, dict) else description,
                source="explore_execute",
                phase_ms=execute_ms,
                total_ms=int((time.perf_counter() - task_started) * 1000),
                exploit_type=description.get("exploit_type") if isinstance(description, dict) else None,
                target_host=description.get("target_host") if isinstance(description, dict) else None,
                target_port=description.get("target_port") if isinstance(description, dict) else None,
                success=description.get("success", False) if isinstance(description, dict) else False,
                request_data=description.get("request_data") if isinstance(description, dict) else None,
                response_data=description.get("response_data") if isinstance(description, dict) else None,
            )
