...
def _looks_like_explore_data(payload: dict[str, Any]) -> bool:
    if not isinstance(payload, dict) or "description" not in payload:
        return False
    allowed_keys = {"description", "exploit_type", "target_host", "target_port", "success", "request_data", "response_data"}
...
def validate_explore_payload(payload: dict[str, Any]) -> tuple[str, dict[str, Any] | None]:
    ...
    # Copy optional exploit detail fields
    for key in ("exploit_type", "target_host", "target_port", "success", "request_data", "response_data"):
